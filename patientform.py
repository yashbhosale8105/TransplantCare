...
class OrganRecipientForm(tk.Tk):
    def __init__(self):
        super().__init__()
        ...
        self.urgency_var = tk.StringVar(value="Select")
    ...
    def create_form(self):
        ...
        # Urgency dropdown beside medical history
        tk.Label(self, text="Urgency:", font=("Trebuchet MS", 16), fg="#000000", bg="#E1FFFD").place(
            relx=0.75, rely=y_offset, anchor="w")
        urgency_menu = tk.OptionMenu(self, self.urgency_var, "Low", "Medium", "High")
        urgency_menu.config(width=20)
        urgency_menu.place(relx=0.75, rely=y_offset + 0.05, anchor="w")
        ...
        # Organ
        tk.Label(self, text="Organ Needed:", font=("Trebuchet MS", 16), fg="#000000", bg="#E1FFFD").place(
            relx=0.75, rely=y_offset, anchor="w")
    ...
    def submit(self):
        try:
            ...
            if any(var.get() in ["", "Select"] for var in [
                ...
                self.organ_var, self.gender_var, self.urgency_var,
                self.state_var, self.city_var
            ]):
            ...
            # Insert data with separate address fields
            cursor.execute("""
            ...
            """, (
                ...
                self.phone_var.get(), 
                self.address_var.get(),  # Street address only
                self.city_var.get(),     # City
                self.state_var.get(),    # State
                self.pincode_var.get(),  # Pincode
                self.blood_group_var.get(), self.organ_var.get(), 
                self.email_var.get(), self.password_var.get(), 
                self.aadhaar_var.get(), self.medical_history_var.get(),
                registration_date, 'Pending', self.urgency_var.get()
            ))
